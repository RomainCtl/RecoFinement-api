...
class TMDB:
    ...
    def get_tokens(request_token):
        ...
        response = requests.post(
            TMDB_URL_TOKEN+TMDB_CLIENT_TOKEN, data=payload)
    ...
    def get_movies_details(id, element):
        details = requests.get(
            TMDB_USER_URL+f"movie/{id}", {"api_key": TMDB_CLIENT_TOKEN}).json()
        ...
        credits = requests.get(
            TMDB_USER_URL+f"movie/{id}/credits", {"api_key": TMDB_CLIENT_TOKEN}).json()
        ...
        for crew in credits['crew']:
            if len(element['producers']) < 5:
                element['producers'].append(
                    crew['name']) if crew['department'] == "Production" else None
    ...
    def get_series_details(id, element):
        details = requests.get(
            TMDB_USER_URL+f"tv/{id}", {"api_key": TMDB_CLIENT_TOKEN}).json()
        ...
        credits = requests.get(
            TMDB_USER_URL+f"tv/{id}/credits", {"api_key": TMDB_CLIENT_TOKEN}).json()
    ...
    def get_favorite_movies(session_id, account_id):
        ...
        response = requests.get(
            TMDB_USER_URL+f"account/{account_id}/favorite/movies", params=params).json()
    ...
    def get_favorite_series(session_id, account_id):
        ...
        response = requests.get(
            TMDB_USER_URL+f"account/{account_id}/favorite/tv", params=params).json()
        data = []
        pages = response['total_pages']
        for page in range(0, pages):
            for item in response["results"]:
    ...
    def get_rated_movies(session_id, account_id):
        ...
        response = requests.get(
            TMDB_USER_URL+f"account/{account_id}/rated/movies", params=params).json()
        data = []
        pages = response['total_pages']
        for page in range(0, pages):
            for item in response["results"]:
                element = {}
                element = TMDB.get_movies(item, element)
                element['user_rating'] = round(
                    float(item['rating']*5/10.0)*2)/2
    ...
    def get_rated_series(session_id, account_id):
        ...
        response = requests.get(
            TMDB_USER_URL+f"account/{account_id}/rated/tv", params=params).json()
        data = []
        pages = response['total_pages']
        for page in range(0, pages):
            for item in response["results"]:
    ...
    def get_created_list(session_id, account_id):
        ...
        response = requests.get(
            TMDB_USER_URL+f"account/{account_id}/lists", params=params).json()
        movies = []
        series = []
        pages = response['total_pages']
        for page in range(0, pages):
            for item in response["results"]:
                userlist = requests.get(
                    TMDB_USER_URL+f"list/{item['id']}", params={"api_key": TMDB_CLIENT_TOKEN}).json()
