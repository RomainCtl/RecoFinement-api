...
class ExternalService:
    ...
    def get_spotify_oauth(user_uuid):
        ...
        try:
            # TODO definir la table pour stock les token access -> modifier if en dessous
            # TODO state parameter against csrf
    ...
    def get_spotify_data(user_uuid, app):
        ...
        with app.app_context():
            ...
            try:
                ...
                data.append(Spotify.get_user_recently_played(external.access_token))
                ...
                for d in data:
                    for line in d :
                        ...
                        if ((meta:=MetaUserTrackModel.query.filter_by(track_id=track.track_id).first()) is None )  : # TODO check played_at
                            new_meta_user_track = MetaUserTrackModel(
                                user_id = user.user_id,
                                track_id = track.track_id,
                                play_count = 1,
                                last_played_date = dateutil.parser.isoparse(line['played_at'][:-1]) if "played_at" in line.keys() else None
                            )
