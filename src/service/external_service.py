...
from src.model import UserModel, ExternalModel, TrackModel, MovieModel, SerieModel, BookModel, MetaUserContentModel, ContentModel
from src import db
        ...
        with app.app_context():
            ...
            try:
                ...
                if "access_token" not in token_info.keys():
                    ExternalModel.query.filter_by(
                        user_id=user.user_id, service_name="Spotify").delete()
                    db.session.commit()
                ...
                for d in data:
                    for line in d:
                        ...
                        if ((meta := MetaUserContentModel.query.filter_by(content_id=track.content_id).first()) is None):
                            new_meta_user_track = MetaUserContentModel(
                                user_id=user.user_id,
                            ...
                            if ((line['played_at'] is not None) and ((meta.last_count_increment is None) or (line['played_at'] > meta.last_count_increment))):
                                meta.last_count_increment = line['played_at']
                db.session.commit()
        ...
        with app.app_context():
            ...
            try:
                ...
                if 'success' in account_id.keys():
                    ExternalModel.query.filter_by(
                        user_id=user.user_id, service_name="TMDB").delete()
                    db.session.commit()
                ...
                for movie in movies:
                    for line in movie:
                        ext_movie = MovieModel.query.filter_by(
                            imdbid=line['imdbid']).first()
                        if ((meta := MetaUserContentModel.query.filter_by(content_id=ext_movie.content_id).first()) is None):
                            new_meta_user_track = MetaUserContentModel(
                                user_id=user.user_id,
                                content_id=ext_movie.content_id,
                                rating=line['user_rating'] if "user_rating" in line.keys(
                                ) else None,
                                watch_count=1,
                                review_see_count=1
                            )
                ...
                for serie in series:
                    for line in serie:
                        serie = SerieModel.query.filter_by(
                            title=line['title'], start_year=line['start_year']).first()
                        if ((serie_db := MetaUserContentModel.query.filter_by(serie_id=serie.serie_id).first()) is None):
                            new_meta_user_track = MetaUserContentModel(
                                user_id=user.user_id,
    ...
    def get_gbooks_data(user_uuid, app):
        ...
        with app.app_context():
            ...
            try:
                ...
                if books == "revoked":
                    ExternalModel.query.filter_by(
                        user_id=user.user_id, service_name="GBooks").delete()
                    db.session.commit()
                ...
                for line in books:
                    if ((meta := MetaUserContentModel.query.filter_by(isbn=line['isbn']).first()) is None):
                        new_meta_user_track = MetaUserContentModel(
                            user_id=user.user_id,
                            isbn=line['isbn'],
